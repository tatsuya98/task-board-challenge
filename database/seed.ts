...
const seed = (boardData: BoardData[], taskData: TaskData[]) => {
    return pool.query("DROP TABLE IF EXISTS tasks")
        .then((_) => {
        ...
        .then((_) => {
            const valuesToInsert: string[][] = boardData.map((obj:BoardData) => [
                obj.board_id,
                obj.board_name,
        ...
        .then((_) => {
            const valuesToInsert: string[][] = tasksData.map((obj:TaskData): string[] => [
                obj.task_id,
                obj.task_name,
            ...
            const insertTasksQuery = format(`INSERT INTO tasks (task_id, task_name, task_description, task_status,
                                                                task_icon_url,
                                                                board_id)
                                             VALUES %L`, valuesToInsert)
